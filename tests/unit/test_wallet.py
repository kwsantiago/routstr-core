...
@pytest.mark.asyncio
async def test_recieve_token_valid():
    token_data = {
        "token": [{
            "mint": "http://mint:3338",
...
async def test_credit_balance():
    ...
    with patch("router.wallet.PRIMARY_MINT_URL", "http://mint:3338"):
        with patch("router.wallet.recieve_token", return_value=(1000, "sat", "http://mint:3338")):
...
@pytest.mark.asyncio  
async def test_credit_balance_invalid_mint():
    mock_key = Mock()
    mock_session = AsyncMock()
    
    with patch("router.wallet.recieve_token", return_value=(1000, "sat", "http://other:3338")):
        with pytest.raises(ValueError, match="Mint URL is not supported"):
            await credit_balance("test_token", mock_key, mock_session)


@pytest.mark.asyncio
async def test_recieve_token_untrusted_mint():
    mock_wallet = Mock()
    ...
    with patch("router.wallet.deserialize_token_from_string") as mock_deserialize:
        ...
        with patch("router.wallet.Wallet.with_db", return_value=mock_wallet):
            mock_wallet.load_mint = AsyncMock()
            with patch("router.wallet.swap_to_primary_mint", return_value=(900, "sat", "http://mint:3338")):
